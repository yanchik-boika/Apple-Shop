...
// Fetch iPad data from the catalog
$result = $mysqli->query("SELECT * FROM catalogShop WHERE category = 'iPad'");
...
<head>
    ...
    <link rel="stylesheet" href="css/iPadShop-style.css">
</head>
<body>
<header>
...
<main>
    ...
    <div class="trying">
        <p>Choose iPad you want to buy.</p>
    </div>
    <div class="iPhone-table">
        <table>
            <?php foreach ($ipads as $i => $ipad): ?>
                ...
                <?php endif; ?>
                <td>
                    <img src="image/iPad1/<?= htmlspecialchars($ipad['image']) ?>" alt="none" class="table-photo<?= $i+1 ?>">
                    <button class="buttton<?= $i+1 ?>"><?= htmlspecialchars($ipad['name']) ?></button><br>
                    <a href="<?= $isUserLoggedIn ? "buyIPadHTML.php?id={$ipad['id']}" : "loginHTML.php" ?>" class="buyNow<?= $i+1 ?>">Buy now</a>
                </td>
                <?php if ($i % 3 === 2): ?>
                    </tr>
                <?php endif; ?>
...
<!--                --><?php //endif; ?>
<!--                <td>-->
<!--                    <img src="image/iPad1/--><?php //= htmlspecialchars($ipad['image']) ?><!--" alt="none" class="table-photo--><?php //= $i+1 ?><!--">-->
<!--                    <button class="buttton--><?php //= $i+1 ?><!--">--><?php //= htmlspecialchars($ipad['name']) ?><!--</button><br>-->
<!--                    <a href="buyIPadHTML.php?id=--><?php //= $ipad['id'] ?><!--" class="buyNow--><?php //= $i+1 ?><!--">Buy now</a>-->
<!--                </td>-->
<!--                --><?php //if ($i % 3 === 2): ?>
<!--                    </tr>-->
<!--                --><?php //endif; ?>
...
<!--    document.querySelectorAll('.button').forEach(button => {-->
<!--        button.addEventListener('click', function() {-->
<!--            const userLoggedIn = --><?php //= json_encode($isUserLoggedIn) ?>//;
<!--//            const url = this.nextElementSibling.getAttribute('href');-->
<!--//            if (!userLoggedIn) {-->
<!--//                window.location.href = 'loginHTML.php';-->
<!--//            } else {-->
